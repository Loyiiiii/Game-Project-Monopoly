...
bool GameBoard::checkPlayer(const string name, const char token) const {
    for (auto& currentPlayer : players) {
        if (currentPlayer.getName() == name || 
            ...
            token != 'G' || token != 'B' || token != 'D' || token != 'P' || token != 'S' || token != '$' || token != 'L' || token != 'T') {
            return true;
        }
    }
    return false;
...
int GameBoard::move(size_t oldpos, size_t newpos) {
    ...
    if (newpos > 40) {
        ...
        cout << "You have passed OSAP, you get $200." << endl;
    } 
    if (newpos < 0) {
        newpos = newpos + 40;
    }
...
void GameBoard::OwnMoney() {
    while (currPlayer->getMoney() < ownMoney) {
        cout << "Please choose trade or mortgage to raise money." << endl;
        string cmd;
        cin >> cmd;
        if (cmd == "trade") {
    ...
    if (ownBank) {
        ownBank = false;
        cout << "You have paid the tuition to bank. Please continue." << endl;
    } else {
        ...
        cout << "You have paid the tuition to the owner. Please continue." << endl;
    }
    ownMoney = 0;
}

void GameBoard::bankrupt() {
    // delete player, clear timscup / totaltimscup, for loop auction
    ...
    if (ownBank) {
        ...
        // give all residence to the owner of the 
    }
}
...
void GameBoard::action(size_t position, int diceSum) {
    ...
    if (checkSquareType(position)) {
        cout << "You have step on a property." << endl;
        ...
        if (buildings[position]->isOwned()) {
            ...
            if (owner == currPlayer->getName()) {
                cout << "You are the owner of this building, please enter your command." << endl;
                return;
            ...
            // Owner is someone else
                ...
                if (isResidence(position)) {
                    ...
                    cout << " You need to pay $" << tuition << " to " << owner << endl;
                    ...
                    if (tuition > currPlayer->getMoney()) {
                        ...
                        cout << "You have paid $" << tuition << " to " << owner << endl;
                    }
        ...
        // no owner
            ...
            cout << "The building is not owned by anyone, do you want to buy it or not, please enter Y/N." << endl;
            string decision;
            while (cin >> decision) {
                if (decision == "Y") {
                    ...
                    cout << "Your input is invalid, please enter Y/N." << endl;
                }
            }
        }
    ...
    // non-property
        ...
        if (position == 2 || position == 17 || position == 33) {
            ...
            if (result == 12345) {
                if (timsCups < 4) {
                    cout << "Congratulation! You got a Roll Up the Rim cup." << endl;
                    currPlayer->addTimsCups(1);
                    timsCups++;
                } else {
                    cout << "Sorry, the number of Roll up the Rim cup is full, please enter your command." << endl;
                }
            } else if (result == -3) {
                ...
                cout << "You are in DC Tims Line now!" << endl;
            } else {
                currPlayer->setPosition(0);
                newpos = move(position, 0);
                currPlayer->addMoney(200);
                cout << "You are in COLLECT OSAP now!" << endl;
            }
            ...
            if (result == -300) {
                if (currPlayer->getMoney() < 300) {
                    cout << "You do not have enough money to pay for the tuition." << endl;
                    ownBank = true;
                    ownMoney = 300;
                    if (isBankrupt()) {
                        bankrupt();
                    ...
                    cout << "You have paid $300 to the bank." << endl;
                }
            } else {
                ...
                if (currPlayer->getMoney() < pay) {
                    ...
                    cout << "You have paid $" << pay << " to the bank." << endl;
                }
            }
            ...
            if (result == -200) {
                if (currPlayer->getMoney() < 200) {
                    ...
                    currPlayer->addMoney(result);
                    cout << "You have paid $200 to the bank." << endl;
                }
            } else if (result == -100) {
                ...
                currPlayer->addMoney(result);
                cout << "You just earned $25." << endl;
            } else if (result == 50) {
                currPlayer->addMoney(result);
                cout << "You just earned $50." << endl;
            } else if (result == 100) {
                currPlayer->addMoney(result);
                cout << "You just earned $100." << endl;
            } else if (result == 200) {
                currPlayer->addMoney(result);
                cout << "You just earned $200." << endl;
            } else {
                if (timsCups < 4) {
                    cout << "Congratulation! You got a Roll Up the Rim cup." << endl;
                    currPlayer->addTimsCups(1);
            ...
            if (currPlayer->getMoney() < 150) {
                cout << "You do not have enough money to pay for the COOP FEE." << endl;
                ownBank = true;
                ownMoney = 150;
                if (isBankrupt()) {
                    bankrupt();
                ...
                cout << "You have paid $150 to the bank." << endl;
            }
        } else if (position == 0) {
            return;

        } else {
            cout << "you are not inside DC Tims Line, nothing happened." << endl;
            return;
        }
    }
}
...
void GameBoard::roll() {
    ...
    if (pos == 10 ) {
        if (currPlayer->getTurn() == 2) {
            cout << "You must pay $50 or use a Roll Up the Rim cup, enter money or cup." << endl;
            string option;
            cin >> option;
            if (option == "money") {
                ...
                if (currPlayer->getTimsCups() <= 0) {
                    cout << "You do not have enough cup, please use money to pay." << endl;
                    if (currPlayer->getMoney() < 50) {
                        cout << "You do not have enough money to pay for the tuition." << endl;
                        ownBank = true;
                        ownMoney = 50;
        ...
        if (doubleroll) {
            ...
            cout << "You could pay $50 or use a Roll Up the Rim cup, enter money, cup, or stay." << endl;
            string option;
            cin >> option;
            if (option == "money") {
                if (currPlayer->getMoney() < 50) {
...
void GameBoard::newGame() {
    cout << "A new Game has been created, please insert the number of player from 2 to 6;" << endl;
    int num = 0;
    cin >> num;
    addPlayer(num);
    // cout << "A new Game has been created, please insert the number of player from 2 to 6;" << endl;
    ...
    //         break;
    //     } else {
    //         cout << "Your input is invalid, please enter a number between 2 to 6" << endl;
    //     }
    // }
    ...
    // char token;
    // for (int i = 0; i < num; i++) {
    //     cout << "Player " << i + 1 << ", please choose your player name." << endl; 
    //     while (cin >> playername) {
    //         if (playername == "BANK") {
    //             cout << "You can not choose BANK as your name, please enter another one." << endl;
    //             continue;
    //         } else {
    //             for (int k = 0; k < i; k++) {
    //                 if (players[k].getName() == playername) {
    //                     cout << "This name is in use, please choose another name." << endl;
    //                     continue;
    //                 }
    //             }
    //             cout << "Player " << i + 1 << ", please choose your token." << endl;
    //             break;
    //         }
    //     }
    //     while (cin >> token) {
    ...
    //             for (int k = 0; k < i; k++) {
    //                 if (players[k].getToken() == token) {
    //                     cout << "The token is in use, please select another one." << endl;
    //                     continue;
    //                 }
    //             }
    //             cout << "You have successfully choose your token." << endl;
    //             break;
    //         } else {
    //             cout << "Please choose a token from G B D P S $ L T, please re-enter." << endl;
    //         }
    //     }
    //     players.emplace_back(playername, token);
    //     buildings[0]->add_player_to_square(token, i);
    // }
...
void GameBoard::loadGame(const string filename) {
    ...
    for (int i = 0; i < playerNumber; i++) {
        ...
        // modify!!!
        players[i] = Player(playername, playerchar, position, money, numcups, timsturn, intims, 0, 0);
    }
        ...
        // building has owner
            ...
            for (int i = 0; i < playerNumber; i++) {
                if (players[i].getName() == owner) {
                    ...
                    players[i].buyProperty(index);
                    players[i].changeMonopolyValue(index, getMonopolyValue(index));
                }
            }
            for (int i = 0; i < 40; i++) {
                if (getBuildingName(i) == buildname) {
                    buildings[i]->setOwner(owner);
                    if (improvements == -1) {
                        int idx = 0;
                        for (int i = 0; i < playerNumber; i++) {
                            if (players[i].getName() == owner) {
                        ...
                        mortgage(i, players[idx]);
                    }

                    Square* temp = buildings[i];
                    Academic *a = dynamic_cast<Academic*> (temp);
                    if (a) {
                        // is academic building
                        buildings[i]->setImprovement(improvements);
                    }
                }
            }
        }
...
void GameBoard::saveGame(const string filename, int curPlayerindex) {
    ...
    for (int i = 0; i < curPlayerindex + 1; i++) {
        bool intims = players[i].getWay();
        string playername = players[i].getName();
        char playerToken = players[i].getToken();
        int playerTimsCups = players[i].getTimsCups();
        int money = players[i].getMoney();
        size_t position = players[i].getPosition();
        int timsturn = players[i].getTurn();
        savefile << playername << " " << playerToken << " " << playerTimsCups << " " << money << " " << position;
        if (position == 10) {
            if (intims) {
                savefile << "1" << timsturn;
            }
