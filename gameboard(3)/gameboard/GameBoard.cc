...
public:
    ...
    int ownMoney = 0;
    bool ownBank = false;
    Player* currPlayer = &players[curidx];
    bool rolled = false;
    ...
    bool rolldice();
    void next();
    int move(size_t oldpos, size_t newpos);
    void OwnMoney();
    void bankrupt();
    void action(size_t position, int diceSum);
    void roll();
    void newGame();
    void loadGame(const string filename);
    void saveGame(const string filename, int curPlayerindex);

    // Basic GameBoard Construction
    GameBoard();
    ~GameBoard();
    bool checkPlayer(const string name, const char token) const;            // 1
    void addPlayer(int numPlayer);                                       // 1
    int countPlayer();                                                      // 1
